...
const getEdu = async () => {
  try {
    const res = await api.post("/get_all_edu");
...
const get_year = async () => {
  try {
    const res = await api.post("/get_all_year");
...
const get_month = async () => {
  try {
    const res = await api.post("/get_all_month");
...
const get_classroom = async () => {
  try {
    const res = await api.post("/get_all_classroom", {
...
watch(
  ...
  (newVal) => {
    ...
    formSearch.value.level = foundClass ? foundClass.level : "";
  },
  { immediate: true }
);
...
const permissionRefs = ref([]);
const absenRefs = ref([]);
const noteRefs = ref([]);

// Assign refs dynamically
const setPermissionRef = (el, idx) => {
  if (el) {
    permissionRefs.value[idx] = el;
  }
};

const setAbsenRef = (el, idx) => {
  if (el) {
    absenRefs.value[idx] = el;
  }
};

const setNoteRef = (el, idx) => {
  if (el) {
    noteRefs.value[idx] = el;
  }
};

// Focus navigation with validation
const focusPrev = (refsArray, idx) => {
  if (idx > 0 && refsArray[idx - 1]) {
    nextTick(() => {
      const element = refsArray[idx - 1];
...
const focusNext = (refsArray, idx) => {
  if (idx + 1 < refsArray.length && refsArray[idx + 1]) {
    nextTick(() => {
      const element = refsArray[idx + 1];
      if (element && typeof element.focus === "function") {
        element.focus();
...
// Clear refs before new data loads
const clearInputRefs = () => {
  permissionRefs.value = [];
  absenRefs.value = [];
  noteRefs.value = [];
};
...
const findData = async () => {
  try {
    ...
    const res = await api.post("/showStudentAttendance", formSearch.value, {
      headers: {
        "Content-Type": "application/json",
      },
    });
    ...
    // Initialize refs arrays after data is set
    await nextTick(); // Ensure DOM is updated
    permissionRefs.value = new Array(studentsAttendance.value.length).fill(
      null
    );
    absenRefs.value = new Array(studentsAttendance.value.length).fill(null);
    noteRefs.value = new Array(studentsAttendance.value.length).fill(null);
  } catch (error) {
    Toast.fire({
...
const addAttendance = async () => {
  ...
  try {
    const res = await api.post("/save_attendance", data, {
      headers: {
        "Content-Type": "application/json",
      },
    });
    ...
    Toast.fire({
      title: error.response?.data?.message || "Error saving attendance",
      icon: "error",
    });
  } finally {
...
<template>
  <div>
    ...
    >
      <v-progress-circular color="green-darken-4" indeterminate />
          ...
          <VCardText v-if="checkData">
            <table style="width: 100%">
              ...
              >
                ...
                <th style="width: 4%">លុប</th>
              </thead>

              <tbody class="customFont">
                <tr v-for="(item, idx) in studentsAttendance" :key="item.id">
                  ...
                  <td class="pa-2">
                    <VTextField
                      :ref="(el) => setPermissionRef(el, idx)"
                      hide-details
                      density="compact"
                      variant="outlined"
                      v-model="item.permission"
                      @keydown.up.prevent="focusPrev(permissionRefs, idx)"
                      @keydown.down.prevent="focusNext(permissionRefs, idx)"
                    />
                  </td>

                  <td class="pa-2">
                    <VTextField
                      :ref="(el) => setAbsenRef(el, idx)"
                      hide-details
                      density="compact"
                      variant="outlined"
                      v-model="item.absen"
                      @keydown.up.prevent="focusPrev(absenRefs, idx)"
                      @keydown.down.prevent="focusNext(absenRefs, idx)"
                    />
                  </td>
                  ...
                  <td class="pa-2">
                    <VTextField
                      ...
                      variant="outlined"
                      v-model="item.note"
                      @keydown.up.prevent="focusPrev(noteRefs, idx)"
                      @keydown.down.prevent="focusNext(noteRefs, idx)"
                    />
                  </td>
