...
class SecondaryExport implements FromView, WithStyles, WithEvents
{
    use Exportable;
        ...
        if ($this->type == 'ខែ' || $this->type == 'month') {
            return [
                AfterSheet::class => function (AfterSheet $event) use ($lastRow, $startRow) {
                    ...
                    // header rank
                    $sheet->getStyle("U6")->getFill()->setFillType(Fill::FILL_SOLID);
                    $sheet->getStyle("U6")->getFill()->getStartColor()->setARGB('FF9933');

                    // body
                    $sheet->getStyle("U7:U{$lastRow}")->getFill()->setFillType(Fill::FILL_SOLID);
                    $sheet->getStyle("U7:U{$lastRow}")->getFill()->getStartColor()->setARGB('FFCC99');
